...
class DrawFrameClass
{
    ...
    public static void mapPrint(int zoom, List<List<string>> fullMap, Player Player1, List<List<System.ConsoleColor>> fullColorMap, string[] onScreenText, List<string[]>[] onScreenTextColor, List<List<System.ConsoleColor>> fullHighColorMap)
    {
        //another i counter
        int j = 0;
                ...
                if (j + 1 <= fullMap.Count)
                {
                    ...
                    if (i < fullMap[j].Count)
                    {
                            ...
                            Player1.charXY[1] + j - ((zoom - 1) / 2) < fullMap[0].Count && Player1.charXY[0] + i - ((zoom - 1) / 2) < fullMap.Count)
                        {
                            ...
                            Console.BackgroundColor = fullHighColorMap[Player1.charXY[1] + j - ((zoom - 1) / 2)][Player1.charXY[0] + i - ((zoom - 1) / 2)];
